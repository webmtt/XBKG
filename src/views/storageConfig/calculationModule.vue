...
import dataCollectionSubmit from './components/addUpdateCalculationModule'
import { getPluginTabList, deletePluginTab, deletePlugin } from '@/api/storage'
  ...
  data() {
    return {
      ...
      tableMeta: [
        ...
        { key: 'inParam', label: '输入参数', width: 200 },
        { key: 'outParam', label: '输出参数', width: 200 },
        { key: 'path', label: '目录属性' },
      ],
      listQuery: {
        page: 1,
    ...
    handleDelete(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          deletePlugin({filePath:row.path}).then(res=>{})
        },
        onCancel: () => {},
      })
    },
