...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      tableMeta: [
        { key: 'S_DEP_AP', label: '计划起飞机场', width: 200 },
        { key: 'S_ARR_AP', label: '计划目的地机场', width: 200 },
        { key: 'S_DEP_TIME', label: '计划起飞时间', width: 200 },
        { key: 'S_ARR_TIME', label: '计划落地时间', width: 200 },
        { key: 'ALDT', label: '实际落地时间' }
      ],
      listQuery: {
        page: 1,
        limit: 10,
      ...
      getOltpFplInfos(postObj)
        .then(response => {
          if (response.code >= 0) {
            ...
            // this.list.forEach(
            // item =>
            //   (item.S_DEP_TIME = moment(item.S_DEP_TIME, 'YYYYMMDDhhmmss').format(
            //     'YYYY-MM-DD HH:mm:ss'
            //   ))
            //   (item.S_ARR_TIME = moment(item.S_ARR_TIME, 'YYYYMMDDhhmmss').format(
            //     'YYYY-MM-DD HH:mm:ss'
            //   ))
            //   (item.ALDT = moment(item.ALDT, 'YYYYMMDDhhmmss').format(
            //     'YYYY-MM-DD HH:mm:ss'
            //   ))
        //   )
